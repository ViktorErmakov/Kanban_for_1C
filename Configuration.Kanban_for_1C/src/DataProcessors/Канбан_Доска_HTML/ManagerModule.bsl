...
Функция ТекстHTMLКанбанДоски() Экспорт
	...
	Для Каждого ДанныеПроекта Из ПроектыДоски Цикл
		...
		ИДПроекта = СтрШаблон("project%1", ДанныеПроекта.ИдентификаторПроектаСтрока);
		
		//Input
//		Цвета = СтрРазделить(ДанныеПроекта.Цвет, ",");
//		КрасныйЦвет = Цвета[0];
//		ЗеленыйЦвет = Цвета[1];
//		СинийЦвет = Цвета[2];
//		ЦветПроекта = СтрШаблон("background-color: rgb(%1, %2, %3);", КрасныйЦвет, ЗеленыйЦвет, СинийЦвет);
		
		УзелИнпутЧекбокса = ДокументHTML.СоздатьЭлемент("input");
		УзелИнпутЧекбокса.УстановитьАтрибут("type", "checkbox");
		УзелИнпутЧекбокса.УстановитьАтрибут("id", ИДПроекта);
		УзелИнпутЧекбокса.УстановитьАтрибут("class", ИДПроекта);
		...
		УзелЛейблЧекбокса.УстановитьАтрибут("for", ИДПроекта);
			
			//Вложенный div
			УзелЧекбоксИмя = ДокументHTML.СоздатьЭлемент("Div");
			УзелЧекбоксИмя.УстановитьАтрибут("class", СтрШаблон("tag %1", ИДПроекта));
			УзелЧекбоксИмя.УстановитьАтрибут("style", СтрШаблон("background-color: rgb(%1);", ДанныеПроекта.Цвет));
			УзелЧекбоксИмя.ТекстовоеСодержимое = Строка(ДанныеПроекта.Проект);
	...
	Для Каждого ДанныеСтатуса Из ДанныеДляДоски.Статусы Цикл
		...
		Для Каждого Задача Из ЗадачиСоСтатусом Цикл
			...
			УзелКарточка.УстановитьАтрибут("class", СтрШаблон("card %1 card__active", Задача.Проект));
