...
Процедура ИспользуемыеСтатусыДоскиЗагрузить()
	...
	Если ЗначениеЗаполнено(ИспользуемыеСтатусы) Тогда
		...
		СтатусыНаФорме = РеквизитФормыВЗначение("СтатусыЗадач");
		Для Каждого СтрокаСтатуса Из ИспользуемыеСтатусы Цикл
			
			НоваяСтрокаПроектыРеквизит = СтатусыНаФорме.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаПроектыРеквизит, СтрокаСтатуса);
			
		КонецЦикла;
		ЗначениеВРеквизитФормы(СтатусыНаФорме, "СтатусыЗадач");
...
Процедура ЗаписатьСтатусыЗадач(Отказ)
	...
	Статусы = канбан_КанбанДоска.НовыйСтатусыДоски();
	
	Для Каждого Статус Из СтатусыЗадач Цикл
		
		НоваяСтрокаСтатус = Статусы.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрокаСтатус, Статус);
