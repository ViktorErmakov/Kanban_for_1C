...
Функция ИзменитьСтатусЗадаче(ИдентификаторЗадачаСтрока, ИдентификаторСтатусаСтрока, Отказ) Экспорт
	...
	МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоСсылке(ЗадачаСсылка);
...
Процедура УстановитьОбъектЗапроса(ТекстЗапроса, ИмяОпределяемогоТипа) Экспорт
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОбъектЗапроса", ПолноеИмяСправочника);
...
Функция ДополнитьЗапросДанныеЗадач(ТекстЗапросаДанныеЗадач, ПараметрыОтбора)
	...
	Оператор.ВыбираемыеПоля.Добавить("УНИКАЛЬНЫЙИДЕНТИФИКАТОР(ЕСТЬNULL(ЗадачиОсновные.Проект, Задачи.Проект))", 8);
