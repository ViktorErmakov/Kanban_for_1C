...
Функция ДополнитьЗапросДанныеЗадач(ТекстЗапросаДанныеЗадач, ПараметрыОтбора)
	...
	Оператор.Источники[0].Соединения.Добавить(
		...
		"узЗадачи.Проект = ИспользуемыеПроектыДоски.Проект И ИспользуемыеПроектыДоски.Пользователь = &ТекущийПользователь");
	
	Оператор.Источники[0].Соединения[2].ТипСоединения = ТипСоединенияСхемыЗапроса.ЛевоеВнешнее; // не обязательно, по умолчанию левое
